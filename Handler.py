...
def handleWeight():
    ...
    _id_ = ""
    if Values.is18b:
        _id_ = "1XuOMbSzI-nfe4ll6v-VbB6s0xFX8Mr28"
    elif Values.is40b:
        _id_ = "1wo60eqh5-HnCSYLmarwujaNNEyBxeRLu"
    elif Values.is60b:
        _id_ = "1d9jmynB2j2f6QkzP7cRkq53mcnBzUQrE"
    else:
        print("Error! Weight file can only choose 40b, 60b and 18b, others are not supported.")
        sys.exit(1)

    if FileUtils.getFileSize("/content/work/data/weights/40b.bin.gz") != WebUtils.getGoogleFileSize(f"{_id_}&confirm=true"):
